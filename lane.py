...
class Lane():
    ...
    #computes mean X values of the previous fits stored as the lane progresses
    def average_pre_lanes(self):
        tmp = copy(self.prev_fitx)
        tmp.append(self.cur_fitx)
        self.mean_fitx = np.mean(tmp, axis=0)

    #if exceeds N deletes the earliest value
...
# fullsize:1280x720 refer warper1.png
x = [194, 1117, 705, 575]
y = [719, 719, 461, 461]
X = [290, 990, 990, 290]
Y = [719, 719, 0, 0]
...
def find_edges(img, s_thresh=s_thresh, sx_thresh=sx_thresh, dir_thresh=dir_thresh):
    ...
    s_channel = hls[:,:,2]
    s_binary = threshold_col_channel(s_channel, thresh=s_thresh) #hls saturation
...
def warper(img, M): #image distortion
...
def full_search(binary_warped, visualization=False):
    ...
    histogram = np.sum(binary_warped[int(binary_warped.shape[0]/2):,:], axis=0) # determine the starting points for the left and right lane lines.
...
def create_output_frame(offcenter, pts, undist_ori, fps, curvature, curve_direction, binary_sub, threshold=0.6):
    ...
    whole_frame = np.zeros((h*3,w*3, 3), dtype=np.uint8) #made a change here from 2.5, 2.34
    ...
    if abs(offcenter) > threshold:
        cv2.putText(whole_frame, "WARNING", (10,20), font, 0.8, (0,0,255), 2)
    ...
    #cv2.putText(whole_frame, "Augmented", (10,20), font, 0.6, (255,0,0), 1, cv2.LINE_AA)
    ...
    cv2.putText(whole_frame, info_warning, (125, 176), font, 0.4, (255,0,0), 1,cv2.LINE_AA)
