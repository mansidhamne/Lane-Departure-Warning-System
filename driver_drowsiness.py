...
def drowsiness_detection(img):
    ...
    alarm_serious = "alarm_serious.wav"
    alarm_beep = ""
    ...
    EYE_AR_THRESH = 0.25
    MOUTH_AR_THRESH_MIN = 0.80
    MOUTH_AR_THRESH_MAX = 0.90
    ...
    while True:
        ...
        # grayscale
        #frame = img.read()
        frame = imutils.resize(img, width=1024, height=576)
        ...
        for rect in rects:
            ...
            if ear < EYE_AR_THRESH:
                ...
                if COUNTER >= EYE_AR_CONSEC_FRAMES:
                    cv2.putText(frame, "DROWSY!", (500, 20),
            ...
            if mar > MOUTH_AR_THRESH_MIN and mar < MOUTH_AR_THRESH_MAX:
                cv2.putText(frame, "YAWNING!", (800, 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            
            if (mar > MOUTH_AR_THRESH_MIN and mar < MOUTH_AR_THRESH_MAX) and ear < EYE_AR_THRESH:
                cv2.putText(frame, "DROWSY: {:.2f}".format(ear), (500, 20),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                cv2.putText(frame, "YAWNING: {:.2f}".format(mar), (450, 20),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                if not ALARM_ON:
                    ALARM_ON = True
                    t = Thread(target=sound_alarm,args=(alarm_serious,))
                    t.deamon = True
                    t.start() 
                    alarm_count += 1
                ALARM_ON = False
